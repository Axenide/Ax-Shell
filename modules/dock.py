    ...
    try:
        ...
        if (
            ...
            for app_id in old_pinned:
                # If app_id is a string, use it directly to look up in app_map
                ...
                # If app_id is already a dict, use it directly
                elif isinstance(app_id, dict):
                    config_data["pinned_apps"].append(app_id)
                else:
                    # Handle unexpected type
                    config_data["pinned_apps"].append({"name": str(app_id)})
            ...
            if not self.integrated_mode and self.get_mapped():
                ...
                if display:
                    ...
                    if gdk_window:
                        ...
                        if not (
                            win_x <= x_ptr <= win_x + width
                            and win_y <= y_ptr <= win_y + height
                        ):
                            ...
                            for i, pinned_app_item in enumerate(self.pinned):
                                if isinstance(app_id_dragged, dict) and isinstance(
                                    ...
                                    if app_id_dragged.get(
                                        "name"
                                    ) == pinned_app_item.get("name"):
                                        app_index_dragged = i
                                        break
